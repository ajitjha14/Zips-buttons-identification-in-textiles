...
# Function to move the robot around the bounding box in counterclockwise sequence
def move_robot_around_corners(bot, corners, loops=1):
    ...
    for loop in range(loops):
        ...
        for i, corner in enumerate(ordered_corners):
            ...
            time.sleep(0.05)  # Shorter sleep for faster movement around the box

    # Return to detecting position after finishing loops
    print("[DEBUG] Returning to detecting position.")
    detecting_home_position(bot)

# Display UI in a separate thread for selecting detected coordinates
...
def display_ui(detection_results, bot):
    ...
    root = tk.Tk()
    root.title("Select Coordinate for Robot")
    ...
    def on_select():
        ...
        if selection:
            ...
            messagebox.showinfo("Coordinate Selected", f"Selected Coordinate - X: {x_world:.3f}, Y: {y_world:.3f}, Z: {z_world:.3f}, Confidence: {confidence:.2f}")
            print(f"Setting robot to selected world coordinates: X={x_world}, Y={y_world}, Z={z_world}")
                    ...
                    for i in range(len(bboxes)):
                        ...
                        if depth_at_center > 0:
                            ...
                            if camera_coords:
                                ...
                                corners = [
                                    transform_point(np.array(calculate_xyz(x, y, depth_frame.get_distance(int(x * x_scale), int(y * y_scale)))), R_matrix, T_vector)
